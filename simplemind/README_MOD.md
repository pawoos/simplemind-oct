...
``` bash
c:\Users\<yourname>> mkdir new_sm
git clone https://gitlab.com/sm-ai-team/simplemind.git
cd simplemind
git checkout mbrown/new_sm
```
### Start WSL2 as root user
``` bash
c:\Users\<yourname>\new_sm\simplemind> wsl -u root
root@usr:/mnt/c/Users/<yourname>/new_sm/simplemind# 
```
...
sudo tar -xvf go1.25.5.linux-amd64.tar.gz
sudo mv go /usr/local
(if does not complete, use: rsync -av go /usr/local)
...
Clean up (delete output and whole working directory) and exit out of smcore:
``` bash
rm -r working-*
exit
```
