...
from . import __available_models__
            ...
            for token in path.split('.'):
                if hasattr(current, token):
                    ...
                    try:
                        ...
                        return None
                elif token in getattr(current, '_modules', {}):
                    current = current._modules[token]
                else:
                    return None
            return current

        for idx in non_trainable_layer_idxs:
            if idx >= len(self.encoder.feature_info):
                continue
            module_path = self.encoder.feature_info[idx]["module"]
            target = resolve(self.encoder, module_path)
            if target is None:
                continue
